    ...
    .then((foundUser) => {
      ...
      bcrypt.compare(password, foundUser.password).then((match) => {
        if (match) {
          ...
          const { _id, name, email, followers, following, pic, bio } =
            foundUser;
          return res.json({
            token,
            user: { _id, name, email, followers, following, pic, bio },
          });
        }

        return res
          .status(422)
          .json({ message: "email and password doesnt match" });
      });
    })
    .catch((err) => {
      console.log(err);
